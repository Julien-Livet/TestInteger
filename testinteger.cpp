...
void TestInteger::testAddition()
{
    ...
    QVERIFY(Integer(0) + 1 == 1);
    QVERIFY(Integer(1) + 2 == 3);
    QVERIFY(Integer(1) + 3 == 4);
    QVERIFY(Integer(-3) + 2 == -1);
    QVERIFY(Integer(17) + 10 == 27);
#ifdef WITH_GMP
    {
        Integer n1, n2;
        n1.setPrecision(4);
    ...
    QVERIFY((Integer(a) + b).cast<long long>() == a + b);
}

void TestInteger::testAnd()
{
    QVERIFY((Integer(0) & 0) == 0);
    QVERIFY((Integer(1) & 0) == 0);
    QVERIFY((Integer(0) & 1) == 0);
    QVERIFY((Integer(1) & 1) == 1);
    ...
    //std::cout << "a b a&b " << a << " " << b << " " << a & b << std::endl;
    QVERIFY((Integer(a) & b).cast<unsigned long long>() == (a & b));
}
...
void TestInteger::testChar()
{
    ...
    QVERIFY(Integer('\0') == '\0');
    QVERIFY(Integer('1') == '1');
    QVERIFY(Integer('a').cast<char>() == 'a');
}

void TestInteger::testDivision()
{
    QVERIFY(Integer(12) / 4 == 3);
    QVERIFY(Integer(2) / 3 == 0);
    QVERIFY(Integer(3) / 2 == 1);
    QVERIFY(Integer(4) / 2 == 2);
    QVERIFY(Integer(-3) / 2 == -1);
    QVERIFY(Integer(-4) / 2 == -2);
    QVERIFY(Integer(5) / 2 == 2);
    QVERIFY(Integer(6) / 2 == 3);
    QVERIFY(Integer(6) / 3 == 2);
    QVERIFY((Integer{122, 17, 200,43} / Integer{23, 117} == Integer{5, 52, 54}));

    {
        Integer const a{4503599627370496ull, 0, 0, 0, 0, 0, 0, 0, 0};
        Integer const b{35521434ull, 14919252733983618111ull, 1302913595559511957ull, 9115028167675518012ull, 17539966127645434035ull};
    ...
    {
        Integer const a{0, 561453, 13205, 1564};
        Integer const b{1698, 721};

        auto const qr{computeQrBurnikelZiegler(a, b)};

        QVERIFY((qr.first == Integer{330, 2815252282}));
        QVERIFY((qr.second == Integer{414, 1722637250}));
    }

    {
        Integer const a("0b10101011110100110001100110101111100010110000011001001100100101110100111011100110100100101110001010001100110111011001101010011011000010111111001010110101010");
        Integer const b("0b100010001011011011011011001110101100100110101011101100101000011011111110010011010101101010101011111110010101010");

        auto const qr{computeQrBurnikelZiegler(a, b)};

        QVERIFY((qr.first == Integer{22110129672729ull}));
        QVERIFY((qr.second == Integer{19184386057769ull, 9219185875676460304ull}));
    }

    {
        Integer const a{122, 17, 200,43};
        Integer const b{23, 117};

        auto const qr{computeQrBurnikelZiegler(a, b)};

        QVERIFY((qr.first == Integer{5, 52, 54}));
        QVERIFY((qr.second == Integer{17, 125}));
    }

    {
        auto const qr1{computeQr(Integer(10), 3)};
        QVERIFY(qr1.first == 3);
        QVERIFY(qr1.second == 1);

        auto const qr2{computeQr(Integer(10), -3)};
        QVERIFY(qr2.first == -4);
        QVERIFY(qr2.second == -2);

        auto const qr3{computeQr(Integer(-10), -3)};
        QVERIFY(qr3.first == 4);
        QVERIFY(qr3.second == -2);

        auto const qr4{computeQr(Integer(-10), 3)};
        QVERIFY(qr4.first == -4);
...
#ifdef WITH_GMP
    {
        ...
        QVERIFY((Integer(a) * b) / c == n.get_str());
    }
#endif
}
...
void TestInteger::testEqualities()
{
    ...
    {
        Integer const R(1000);
        Integer const R_(247), n_(843);
...
{
    QVERIFY(Integer((short)0) >= Integer{(short)0});
    QVERIFY(Integer((short)0) <= Integer{(short)0});
    QVERIFY(Integer((short)1) > Integer{(short)0});
    QVERIFY(Integer((short)-1) < Integer{(short)0});
    QVERIFY(Integer(15) < 16);
    QVERIFY(Integer(16) > 15);
    QVERIFY(Integer(0) < 1);
    QVERIFY(Integer(1) > 0);
    QVERIFY(Integer(1) < 2);
    QVERIFY(Integer(3) > 2);
...
{
    QVERIFY(Integer(2) % 2 == 0);
    QVERIFY(Integer(4) % 2 == 0);
    QVERIFY(Integer(5) % 3 == 2);
    QVERIFY(Integer(-5) % -3 == -2);
    QVERIFY(Integer(-5) % 3 == 1);
    QVERIFY(Integer(5) % -3 == -1);
    QVERIFY(Integer(5) % 1 == 0);
...
void TestInteger::testMultiplication()
{
    ...
    QVERIFY(Integer(-1) * 1 == -1);
    QVERIFY(Integer(-1) * -1 == 1);
    QVERIFY(Integer(5) * 7 == 35);
    QVERIFY(Integer(-5) * 7 == -35);
    QVERIFY(Integer(5) * -7 == -35);
    QVERIFY(Integer(-5) * -7 == 35);
    QVERIFY(-Integer(5) * -7 == 35);
    QVERIFY(Integer(827382986) * 2670051752 == 2209155391344291472ull);
    QVERIFY(Integer(17539966127645434034ull) * 452240028370ull == Integer("7932274779175210128837123544580"));
...
#ifdef WITH_GMP
    {
        ...
        QVERIFY(Integer(a) * b == n.get_str());
    }

    {
        Integer const a{0ull, 0ull, 35521434ull, 14919252733983618111ull};
        Integer const b{11163967620057660708ull, 5245663108828415198ull, 4063525853430116991ull, 8147888677444386816ull};
    ...
    {
        Integer const a{330, 2815252282};
        Integer const b{1698, 721};
...
{
    QVERIFY(pow(Integer(0), 0) == 1);
    QVERIFY(pow(Integer(1), 0) == 1);
    QVERIFY(pow(Integer(2), 0) == 1);
    QVERIFY(pow(Integer(0), 1) == 0);
    QVERIFY(pow(Integer(0), 2) == 0);
    QVERIFY(pow(Integer(1), 0) == 1);
    QVERIFY(pow(Integer(1), 1) == 1);
    QVERIFY(pow(Integer(1), 2) == 1);
    QVERIFY(pow(Integer(2), 2) == 4);
    QVERIFY(pow(Integer(2), 3) == 8);
    QVERIFY(pow(Integer(3), 2) == 9);
    QVERIFY(powm(Integer(5), 13, 23) == 21);
}
...
void TestInteger::testPrimes()
{
...

    {
        ...
        QVERIFY(mpz_jacobi(n1.get_mpz_t(), n2.get_mpz_t()) == jacobi(Integer(n1), n2));
    }
#endif
...
{
    QVERIFY((Integer(1) >> 1) == 0);
    QVERIFY((Integer(3) >> 1) == 1);
    QVERIFY((Integer(1) << 1) == 2);
    QVERIFY((Integer(1878050631) << 31) == 4033083020188581888ull);
    QVERIFY((Integer(1800090071576084480ull) >> 21) == 858349834240ull);
...
{
    QVERIFY(!Integer((short)0));
    QVERIFY(Integer((short)0) == (short)0);
    QVERIFY(Integer((short)1) == (short)1);
    QVERIFY(Integer((short)-1) == (short)-1);
    QVERIFY(Integer((short)0).cast<short>() == (short)0);
    QVERIFY(Integer((short)1).cast<short>() == (short)1);
    QVERIFY(Integer((short)-1).cast<short>() == (short)-1);
}

void TestInteger::testString()
{
    QVERIFY((Integer("0b10", 2) == 2));
    QVERIFY((Integer("17", 10) == 17));
    QVERIFY((Integer("f", 16) == 15));
    QVERIFY((Integer("Z", 62) == 61));
}
...
void TestInteger::testSubstraction()
{
    QVERIFY(Integer(-1) - 2 == -3);
    QVERIFY(Integer(-1) - 3 == -4);
    QVERIFY(Integer(-2) - 4 == -6);
    QVERIFY(Integer(0) - 1 == -1);
    QVERIFY(Integer(1) - 1 == 0);
    QVERIFY(Integer(3) - 1 == 2);
    QVERIFY(Integer(1) - 4 == -3);
    QVERIFY(Integer(-5) - -3 == -2);
    QVERIFY(Integer(3) - 2 == 1);
    QVERIFY(Integer(17) - 10 == 7);
...
{
    QVERIFY(Integer('\2').toString(2) == "0b00000010");
    QVERIFY(Integer((int)2).toString(2) == "0b00000010");
    QVERIFY((Integer('\2') >> 1).toString(2) == "0b00000001");
    QVERIFY((Integer('\2') >> 2).toString(2) == "0b00000000");
    QVERIFY((Integer((unsigned char)255) << 1).toString(2) == "0b0000000111111110");
    QVERIFY(((Integer((unsigned char)255) << 1) >> 2).toString(2) == "0b01111111");
    QVERIFY(Integer(10).toString() == "10");
    QVERIFY(Integer(17).toString() == "17");
    QVERIFY(Integer(15).toString(16) == "0xf");
    QVERIFY(Integer(10).toString(62) == "a");
    QVERIFY(Integer(35).toString(62) == "z");
    QVERIFY(Integer(36).toString(62) == "A");
    QVERIFY(Integer(61).toString(62) == "Z");
    QVERIFY(Integer("94882770404074832568499").toString() == "94882770404074832568499");
}

void TestInteger::testUnsignedLongLong()
{
    QVERIFY(!Integer(0ull));
    QVERIFY(Integer(1ull) == 1);
    QVERIFY(Integer(1ull).cast<unsigned long long>() == 1ull);
...
{
    QVERIFY(!Integer((unsigned short)0));
    QVERIFY(Integer((unsigned short)0) == (unsigned short)0);
    QVERIFY(Integer((unsigned short)1) == (unsigned short)1);
    QVERIFY(Integer((unsigned short)-1) == (unsigned short)-1);
    QVERIFY(Integer((unsigned short)0).cast<unsigned short>() == (unsigned short)0);
    QVERIFY(Integer((unsigned short)1).cast<unsigned short>() == (unsigned short)1);
    QVERIFY(Integer((unsigned short)-1).cast<unsigned short>() == (unsigned short)-1);
}
